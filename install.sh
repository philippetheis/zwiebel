...
if [ ! -f "install-init" ]; then
	...
	echo "$(date +%s):">>install.log;
	echo "zwiebel Installation gestartet"|tee -a install.log;
	...
	echo "$(date +%s):">>install.log;
	echo "aendere Rechnernamen auf zwiebel"|tee -a install.log;

	echo "$(date +%s):">>install.log;
	sudo sed -i 's/127.0.1.1.*raspberry.*/127.0.1.1 zwiebel/' /etc/hosts|tee -a install.log;

	echo "$(date +%s):">>install.log;
	sudo bash -c "echo 'zwiebel' > /etc/hostname"|tee -a install.log;
...
echo "$(date +%s):">>install.log;
echo "fuehre Installation des zwiebel fort"|tee -a install.log;
...
sudo cp apache_zwiebel.conf /etc/apache2/sites-available/|tee -a install.log;
sudo chmod 644 /etc/apache2/sites-available/apache_zwiebel.conf|tee -a install.log;

echo "$(date +%s):">>install.log;
sudo a2dissite default|tee -a install.log;
sudo a2ensite apache_zwiebel.conf|tee -a install.log;
...
echo "$(date +%s):">>install.log;
echo "Installation war erfolgreich. Ihr zwiebel ist gleich im Browser ueber die Adresse 'zwiebel.local' erreichbar"|tee -a install.log;
echo "Verbinden Sie sich dafuer mit dem WLAN namens zwiebel. Das Standard-WPA-Passwort ist stets spiegelonline.";
...
echo "Bitte starten Sie ihren zwiebel jetzt neu (unter Umstaenden sogar mehrmals), indem Sie den Strom unterbrechen"|tee -a install.log;
